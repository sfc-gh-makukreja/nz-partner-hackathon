...
SELECT 
    port_code,
    EXTRACT(HOUR FROM tide_datetime) as hour_of_day,
    tide_sequence,
    COUNT(*) as frequency,
    ROUND(AVG(tide_height_m), 2) as avg_height_m
FROM tide_predictions 
WHERE date >= '2024-01-01'
GROUP BY port_code, EXTRACT(HOUR FROM tide_datetime), tide_sequence
...
SELECT 
    ...
    MONTHNAME(prediction_date) as month_name,
    ROUND(MAX(CASE WHEN tide_type = 'High' THEN tide_height_m END), 2) as highest_high_tide_m,
    ROUND(MIN(CASE WHEN tide_type = 'Low' THEN tide_height_m END), 2) as lowest_low_tide_m,
    ROUND(MAX(CASE WHEN tide_type = 'High' THEN tide_height_m END) - 
          MIN(CASE WHEN tide_type = 'Low' THEN tide_height_m END), 2) as max_tide_range_m
FROM tide_predictions 
GROUP BY port_code, EXTRACT(YEAR FROM prediction_date), EXTRACT(MONTH FROM prediction_date), MONTHNAME(prediction_date)
...
SELECT 
    ...
    tide_time,
    tide_height_m,
    LAG(tide_time) OVER (PARTITION BY port_code, prediction_date ORDER BY tide_time) as prev_tide_time,
    DATEDIFF('minute', 
        LAG(tide_time) OVER (PARTITION BY port_code, prediction_date ORDER BY tide_time), 
        tide_time) as minutes_since_last_tide
FROM tide_predictions 
WHERE tide_type = 'High' 
  AND tide_height_m > 2.5
  AND prediction_date BETWEEN '2024-01-01' AND '2024-01-31'
...
SELECT 
    ...
    tide_height_m,
    DATEADD('hour', -2, tide_time) as fishing_window_start,
    DATEADD('hour', 2, tide_time) as fishing_window_end,
    CASE 
        WHEN EXTRACT(HOUR FROM tide_time) BETWEEN 5 AND 8 THEN 'Morning Prime Time'
        WHEN EXTRACT(HOUR FROM tide_time) BETWEEN 17 AND 20 THEN 'Evening Prime Time'
        ELSE 'Standard Time'
    END as fishing_quality
FROM tide_predictions 
WHERE tide_type = 'High'
  AND prediction_date BETWEEN CURRENT_DATE() AND DATEADD('day', 7, CURRENT_DATE())
ORDER BY port_code, prediction_date, tide_time;
...
-- Query the tidal_range_analysis view
SELECT 
    port_code,
    analysis_date,
    high_tide_height_m,
...
FROM tidal_range_analysis 
WHERE analysis_date >= '2024-01-01'
  AND range_category IN ('High Range', 'Very High Range')
ORDER BY daily_range_m DESC
LIMIT 20;

-- Query the tide_navigation_windows view  
SELECT *
FROM tide_navigation_windows 
WHERE port_code LIKE '%070%'  -- Auckland
  AND optimal_window_start >= CURRENT_DATE()
ORDER BY optimal_window_start
LIMIT 10;
...
SELECT 
    ...
    tide_type,
    -- Time-based features
    EXTRACT(HOUR FROM tide_time) as hour,
    EXTRACT(DAYOFWEEK FROM prediction_date) as day_of_week,
    EXTRACT(DAYOFYEAR FROM prediction_date) as day_of_year,
    EXTRACT(WEEK FROM prediction_date) as week_of_year,
    -- Lag features
    LAG(tide_height_m, 1) OVER (PARTITION BY port_code ORDER BY prediction_date, tide_time) as prev_tide_height,
    LAG(tide_height_m, 2) OVER (PARTITION BY port_code ORDER BY prediction_date, tide_time) as prev_2_tide_height,
    -- Rolling statistics
    AVG(tide_height_m) OVER (
        PARTITION BY port_code 
        ORDER BY prediction_date, tide_time 
...
WITH tide_cycles AS (
    SELECT 
        port_code,
        prediction_date,
        COUNT(CASE WHEN tide_type = 'High' THEN 1 END) as daily_high_tides,
        COUNT(CASE WHEN tide_type = 'Low' THEN 1 END) as daily_low_tides,
        MAX(tide_height_m) - MIN(tide_height_m) as daily_range,
        EXTRACT(HOUR FROM MIN(CASE WHEN tide_type = 'High' THEN tide_time END)) as first_high_hour
    FROM tide_predictions 
    GROUP BY port_code, prediction_date
)
SELECT 
    port_code,
    daily_high_tides,
    ...
    CASE 
        WHEN daily_high_tides = 2 AND daily_low_tides = 2 THEN 'Semi-diurnal (typical)'
        WHEN daily_high_tides = 1 AND daily_low_tides = 1 THEN 'Diurnal'
        ELSE 'Mixed/Irregular'
    END as tide_pattern_type
FROM tide_cycles
GROUP BY port_code, daily_high_tides, daily_low_tides,
    CASE 
        WHEN daily_high_tides = 2 AND daily_low_tides = 2 THEN 'Semi-diurnal (typical)'
        WHEN daily_high_tides = 1 AND daily_low_tides = 1 THEN 'Diurnal'
        ELSE 'Mixed/Irregular'
    END
ORDER BY port_code, daily_high_tides DESC;
...
SELECT 
    ...
    tide_height_m,
    tide_type,
    CONCAT(EXTRACT(YEAR FROM prediction_date), '-', 
           LPAD(EXTRACT(MONTH FROM prediction_date), 2, '0'), '-',
           LPAD(EXTRACT(DAY FROM prediction_date), 2, '0'), 'T',
           LPAD(EXTRACT(HOUR FROM tide_time), 2, '0'), ':',
           LPAD(EXTRACT(MINUTE FROM tide_time), 2, '0'), ':00Z') as iso_datetime
FROM tide_predictions 
